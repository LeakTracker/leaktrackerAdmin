                    ...
                    <div class="card-header pb-0">
                        <div class="form-group">
                            <div style="width:300px !important;">
                                <label for="inputAddress2">Filter by status</label>
                                <select v-model="statusFilter" @change="setFilter()" class="form-select"
                                    ...
                                    <tr v-for="(data, index) in reports" :key="index" :value="data">
                                        <td>
                                            <div class="d-flex px-2 py-1">
                                                <div>
                                                    <img :src="data.image_link" class="avatar avatar-sm me-3" alt="user1" />
                                                </div>
                                        ...
                                        <td class="align-middle text-center">
                                            <p class="text-xs font-weight-bold mb-0">
                                                <span v-if="data.status == 'Submitted'" class="badge badge-warning">{{
                                                    data.status }}</span>
                                                <span v-if="data.status == 'Confirmed'" class="badge badge-success">{{
                                                    data.status }}</span>
                                                <span v-if="data.status == 'Repaired'" class="badge badge-primary">{{
                                                    data.status }}</span>
                                            </p>
                                        </td>
                                        <td class="align-middle text-center">
                                            <p class="text-xs text-secondary mb-0">{{
                                                data.time_reported.toDate("dd/MM/yyyy").toDateString() }}</p>
                                        </td>
                                        <td class="align-middle text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ data.pipe_size }}</p>
                                        </td>
                                        <td class="align-middle text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ data.water_loss.toLocaleString() }}
                                                Liters</p>
                                        </td>
                                        <td class="align-middle text-center">
                                            <span class="text-secondary text-xs font-weight-bold">{{ data.address }}</span>
                                        </td>
                                        <td class="align-middle text-center">
        ...
        <div class="modal-dialog modal-dialog-    " role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div :key="renderComp">
                        <!-- <div v-html="htmlMaps"></div> -->
                        <!-- {{ htmlMaps }} -->
                        <!-- {{ {htmlMaps} }} -->
                        <GMapMap :center="center" :zoom="15" map-type-id="terrain" style="width: 450px; height: 300px">
                            <GMapCluster>
    ...
    methods: {
        async getAll() {
            ...
            if (this.statusFilter == "All") {
                ...
                const accRef = collection(db, "Reports");
                const querySnapshot = await getDocs(query(accRef, where("status", "==", this.statusFilter), orderBy("timestamp", "desc")));
                querySnapshot.forEach((doc) => {
                    all_reports.push(doc.data());
                    ids.push(doc.id);
                });
